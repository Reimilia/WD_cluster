...
HE_edge* Mesh3D::InsertEdge(HE_vert* vstart, HE_vert* vend)
{
	...
	{
		//std::cout << "Warning : repeated edge at vpair (" << vstart->id() << ',' << vend->id() << ')' << std::endl;
...
bool Mesh3D::LoadFromOBJFile(const char* fins)
{
	...
	try
	{
		...
		std::cout << pvertices_list_->size() << " vertex, " << pfaces_list_->size() << " faces " << std::endl;
...
Mesh3D::Mesh3D(Mesh3D * src_)
{
	ClearData();
	
	for (int i = 0; i < src_->num_of_vertex_list(); i++)
	{
		InsertVertex(src_->get_vertex(i)->position());
	}

	std::vector<HE_vert*> tri;
	for (int i = 0; i < src_->num_of_face_list(); i++)
	{
		src_->get_face(i)->face_verts(tri);
		InsertFace(tri);
	}
	UpdateMesh();
}
